 ...
 * 리포지토리의 용도:
1). 2인 이상의 팀을 꾸려 코드를 작성할 때 활용된다. 이때 소스 코드에 손상을 가하지 않으면서 소프트웨어의 프로그램 코드를 여러 번 변경할 수 있다.
2). 프로젝트 코드를 공유하고 다른 사람의 코드를 확인할 수 있다. 이때 애플리케이션의 프로덕션 버전에 영향을 미치지 않고 새 기능 또는 버그 수정을 위해 리포지토리를 사용한다.
